...
def take_last_day_and_calculate_daily_stats():
    ...
    last_day_df = pd.read_sql(f"SELECT * FROM lucjan_data WHERE date BETWEEN '{start_of_yesterday}' AND '{start_of_today}'", conn)
    insert_row = datetime.datetime.strptime(start_of_yesterday, '%Y-%m-%d'), wakeup(last_day_df), online_today(last_day_df), pd.to_datetime(start_of_yesterday).strftime('%A')
